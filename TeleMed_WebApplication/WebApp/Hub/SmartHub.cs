...
namespace WebApp.Hub
{
    ...
    {
        string sApiHostAddress = ConfigurationManager.AppSettings["ApiBaseUrl"].ToString();
        ...
        public void SendMessageToDoctor(MessageInfo message)
        {
            ...
            if (message.Message.ToString().Contains("call"))
            {
                using (var wb = new System.Net.WebClient())
                {
                    ...
                    var response = wb.UploadValues(sApiHostAddress+"/api/Account/SendPush?docID=" + message.ReceiverId + "&patID=" + message.SenderId + "&sendtoPatient=" + false + "&sendtoDoctor=" + true + "&message=Incoming Consultation Call.", "POST", data);
                }
            }
        ...
        public void SendMessageToPatient(MessageInfo message)
        {
            ...
            if (message.Message.ToString().Contains("consultID"))
            {
                using (var wb = new System.Net.WebClient())
                {
                    ...
                    var response = wb.UploadValues(sApiHostAddress + "/api/Account/SendPush?docID=" + message.SenderId + "&patID=" + message.ReceiverId + "&sendtoPatient=" + true + "&sendtoDoctor=" + false + "&message=Incoming Consultation Call.", "POST", data);
                }
            }
        }
