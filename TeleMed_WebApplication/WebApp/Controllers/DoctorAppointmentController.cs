...
namespace WebApp.Controllers
{
    ...
    public class DoctorAppointmentController : Controller
    {
        ...
        //Felix
        public ActionResult Doctor_Prescription(long? appID)
        {
            long apID = Convert.ToInt64(appID);
        ...
        public PartialViewResult DoctorPrescription()
        {
            try
            {
                var oData = oAppointmentRepository.GetUpcomingApp(SessionHandler.UserInfo.Id);
                ViewBag.errorMessage = "";
                ViewBag.successMessage = "";
                return PartialView("DoctorPrescription", oData);
            ...
            catch (System.Web.Http.HttpResponseException ex)
            {
                ...
                return PartialView("DoctorPrescription");
            }

        }


        public JsonResult GetDrugSearchResult(string P_Name)
        {
            try
            {
                List<GetPrescriptionDrug> Response = new List<GetPrescriptionDrug>();
                //DoseSpotRepository DoseSpotRepository = new DoseSpotRepository();
                Response = oAppointmentRepository.GetDrugSmartSearchResult(P_Name).ToList();
                return Json(Response, JsonRequestBehavior.AllowGet);
                //return Json(new { Success = true, Object = Response });
            }
            catch (Exception ex)
            {
        ...
        //Felix
        [HttpPost]
        public JsonResult GetDrugDescription(string prefix)
        {
            try
            {
                //MedicationRepository objRepo = new MedicationRepository();
                List<GetPrescriptionDrug> model = oAppointmentRepository.GetDrugDescription(prefix);
        ...
        [HttpPost]
        public JsonResult SaveDrugPrescription(List<SaveDrugPrescription> arr)
        {
            try
            {
                ApiResultModel model = oAppointmentRepository.SavePrescription(arr);

                return Json(new { Success = true, ApiResultModel = model });
        ...
        [HttpPost]
        public JsonResult XmlDataConvesion(CancelRescheduleRequestModel AppId)
        {
            try
            {
                ApiResultModel model = oAppointmentRepository.XmlDataConversion(AppId);
        ...
        public JsonResult GetPrescriptionFrequency()
        {
            try
            {

                List<GetPrescriptionFrequency> model = oAppointmentRepository.GetPrescriptionFrequency();
