...
function fetchTimings(fetchdate) {
    ...
    // edit anita
    var myDate = new Date(new Date(fetchdate));
    ...
    var tempfetchdate = formatDate(new Date(localDate), "dd/MM/yyyy");  

    var _objSearch = {};
    _objSearch["appDate"] = tempfetchdate.trim();//$("#fetchdate").val();
    // edit end 
        ...
        success: function (response) {
            ...
            else
            {
                ...
                $.each(response.Object, function (item) {
                    ...
                    var flag = compareDateTime(tempfetchdate.trim(), response.Object[item]);
                    if (flag)
                    {
                        ...
                        tablehtml = tablehtml + " <li><button id ='" + response.Object[item] + "' type='button' class='btn btn-primary' onclick='setDateTime(\"" + response.Object[item] + "\",\"" + tempfetchdate + "\")' style='width:85px'>" + response.Object[item] + "</button></li>";
                    }
...
function showDoctorTimings(doctorID) {
    ...
    // edit anita
    var tempdt = dd + '/' + mm + '/' + yyyy;
    //alert(tempdt);
    _objSearch["appDate"] = tempdt;
